...
SELECT P.LocationName, Left(E.EventName,4) + Right(E.EventName,15) AS Event, E.Temperature_C,  W.Code AS Wind, R.Code AS Precip, E.Clouds_pct AS Percent_Cloud, N.Code AS Noise, I.Name AS Interval, 
...
WHERE (E.EventDateTime > CAST('20150101' AS datetime)) AND 
     ((LEFT(P.LocationName,4) = 'HEHO') OR 
     (LEFT(P.LocationName,4) = 'GWCA') OR 
	 (LEFT(P.LocationName,4) = 'HOME') OR
	 (LEFT(P.LocationName,4) = 'EFMO') OR
	 (LEFT(P.LocationName,4) = 'TAPR') OR
	 (LEFT(P.LocationName,4) = 'PIPE') OR
